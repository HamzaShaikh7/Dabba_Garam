...
def postgresql_check():
    ...
    try:
        ...
        select_date = '''SELECT DISTINCT(date) FROM delivery_report WHERE month = %s ORDER BY date desc'''
        month = [last_month]
        ...
        select_outlet = '''SELECT DISTINCT(outlet) FROM delivery_report WHERE month = %s AND date = %s ORDER BY outlet'''
