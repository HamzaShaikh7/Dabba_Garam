...
def postgresql_check():
    ...
    try:
        ...
        select_date = '''SELECT DISTINCT(order_date) FROM order_wise_cunsumption ORDER BY order_date desc LIMIT 1;'''

        cur.execute(select_date)
        Result=cur.fetchall()

        print(f'Last Updated date ::== {str(Result)[16:-4]}')

        select_outlet = '''SELECT DISTINCT(TRIM(outlet)) FROM order_wise_cunsumption WHERE order_date = %s ORDER BY TRIM(outlet)'''
